...
,where Gamma is the group generated by the AL operators in auts, satisfied
rk(J(C)(\Q))=rk(J(X)(\Q)); set divs of degree 0 divisors that generate J(X)(\Q), 
an abstract abelian group A isomorphic to J(X)(\Q) such that A.i corresponds to divs[i]
...
if not p eq 2 then assert Dimension(T) eq Genus(X) - genusC; end if; //cf Remark XX.
...
//This function verifies the conditions of Theorem XX. It is very similar to
//the previous function.
IsOnlyWithFamily:=function(QQ,p)
ptlist:=[];
d:=2;
...
/*This function translates the info from IsLonely and IsWithFamily
...
I:=1; //I must satisfy I*J_X(\Q) \subset G.
if GCD(#JFp,I) eq 1 then 
pos:={DD : DD in degr2 | (JFp!psi(DD-bpp)) in imG}; //The points in X^{(2)}(\F_p) mapping to the image of J_X(\Q).
else pos:={DD : DD in degr2 |(JFp!(I*psi(DD-bpp))) in imG};
end if;
...
for DD in L do
if #auts eq 1 and IsPullback(DD) then //Case of Theorem XX of the paper.
//#auts eq 1 condition excludes N=57.
if not IsOnlyWithFamily(DD,p) then posP:=posP join {reduce(X,Xp,DD)};
end if;
else if not IsLonely(DD,p) then posP:=posP join {reduce(X,Xp,DD)}; //Case of Theorem XX.
end if; end if;
...
sieve based on the heuristics described in Section XX of the paper. 
...
PrimeSelector:=function(B,iA,primes,exclprimes,jacs,divlist)
ratios:=[];
...
ind:=PrimeSelector(B,iA,primes,exclprimes,jacs,divlist); 
p:=primes[ind]; // We select the next prime.
Append(~exclprimes,p);
p;
Wp,Bp,iAp:=ChabautyInfo(L,p);
